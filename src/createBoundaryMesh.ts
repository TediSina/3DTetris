 ...
 */
export function createBoundaryMesh(scene: BABYLON.Scene, width: number, height: number, depth: number, transparencyFaces: number[] = []) {
    const materials = [];
    ...
    const boundaryMesh = BABYLON.Mesh.MergeMeshes(faces, true, true, undefined, false, true);
